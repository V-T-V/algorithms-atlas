...
# ---- 3. Pancake sort variant: max-to-end (standard reversal-based selection) ----
SORTING.append(dict(
    id="sort-pancake-min",
    zh="煎饼排序（选极值翻转）",
    en="Pancake Sort (Selection-Flip)",
    szh="每轮用翻转把当前未排序段的最大值送到段尾，n-1 轮完成排序。",
    sen="Each round use flips to send the current max of the unsorted segment to its tail; n-1 rounds.",
    dzh="煎饼排序（Pancake Sort）只允许用「反转前 k 个」操作（flip(k)）。本实现每轮在未排序段 a[0..size) 中找到最大值的下标 mi：先 flip(mi) 把它翻到数组顶部（下标 0），再 flip(size-1) 把它从顶部翻到当前段尾，固定该位置，size 减一。共 n-1 轮，每轮最多 2 次 flip。比较次数 O(n^2)，翻转次数最多 2(n-1)。不稳定，原地。",
    den="Pancake sort only allows reversing the first k elements via flip(k). This implementation finds the index mi of the maximum in the unsorted segment a[0..size): flip(mi) brings it to the top (index 0), then flip(size-1) moves it from the top to the current segment tail, fixing that position; size decreases. n-1 rounds, at most 2 flips each: O(n^2) comparisons, at most 2(n-1) flips. Unstable, in-place.",
    tags="['sorting', 'comparison', 'in-place', 'flip']",
    time="O(n^2)", space="O(1)",
    impl="""// 煎饼排序（选极值翻转）· 纯算法实现
...
export function pancakeSortMin(arr: readonly number[], hooks: PancakeMinHooks = {}): number[] {
  ...
  for (let size = n; size > 1; size--) {
    ...
    if (mi !== size - 1) {
      if (mi !== 0) { flip(a, mi); hooks.onFlip?.(mi, a); }
...
# run(SORTING) -- moved to end
import sys, os
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from gen_sorting_more import add_more, add_more2
from gen_sorting_part3 import add_part3
add_more(SORTING, gen, std_bar_trace, std_test, write, ROOT, TEST_ROOT, esc)
add_more2(SORTING, gen, std_bar_trace, std_test, write, ROOT, TEST_ROOT, esc)
add_part3(SORTING, gen, std_bar_trace, std_test, write, ROOT, TEST_ROOT, esc)
run(SORTING)
