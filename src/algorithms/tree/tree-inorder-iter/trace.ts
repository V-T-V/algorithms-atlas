...
import { inorderIter, buildTree, type BTNode, type InorderIterHooks } from './impl.ts';
...
export function buildTrace(input: Array<number | null> = DEFAULT_INPUT): Frame[] {
  ...
  rec
    .begin({
      zh: '初始二叉树，中序遍历（左→根→右）',
      en: 'Initial tree; inorder (left→root→right)',
    })
    .setTree(toViz(root, visited, current) ?? { id: 'empty', value: '∅' })
    .setAux([{ label: '栈', value: '[]', role: 'pivot' }])
    .commit();

  const hooks: InorderIterHooks = {
    onPush: (v) => stackVals.push(v),
    onPop: (v) => {
