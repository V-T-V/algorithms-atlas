...
import { linearProbing, type LinearProbingHooks, type Slot } from './impl.ts';
...
export function buildTrace(input: number[] = DEFAULT_INPUT, size: number = DEFAULT_SIZE): Frame[] {
  ...
  snapshot({
    zh: `哈希表大小 ${size}（hash = key % ${size}），线性探测步长 1，插入序列：${input.join(', ')}`,
    en: `Table size ${size} (hash = key % ${size}), linear step 1, insert sequence: ${input.join(', ')}`,
  });

  const hooks: LinearProbingHooks = {
    onHash: (key, slot) => {
      curHash = slot;
      probeSlots = new Set();
      snapshot({
    ...
    onCollision: (slot) => {
      snapshot({
        zh: `槽 ${slot} 已占用（冲突），探测下一槽 (slot+1)`,
        en: `Slot ${slot} occupied (collision), probe next (slot+1)`,
      });
    },
    onInsert: (key, slot) => {
      slots[slot] = key;
  ...
  linearProbing(input, size, hooks);
