// hash-whirlpool-impl · 录制帧序列
import type { BarRole, Frame } from '../../../types.ts';
import { TraceRecorder } from '../../../core/recorder.ts';
import { hashWhirlpoolImpl } from './impl.ts';
...
export function buildTrace(input: string | readonly number[] = DEFAULT_INPUT): Frame[] {
  ...
  rec
    .begin({
      zh: `hash-whirlpool-impl ${bytes.length} 字节`,
      en: `hash-whirlpool-impl ${bytes.length} bytes`,
    })
    .setAux([{ label: '字节', value: String(bytes.length), role: 'pivot' as BarRole }])
    .commit();
  let r: bigint[] = [];
  hashWhirlpoolImpl(input, {
    onBlock: (i) =>
      rec
