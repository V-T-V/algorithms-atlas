// =============================================================================
// SLR(1) 分析器 · 录制帧序列
// 用 setGrid 展示 LR(0) 项目集规范族 + ACTION/GOTO 表；setAux 展示分析栈。
// =============================================================================

import type { BarRole, Cell, Frame } from '../../../types.ts';
import { TraceRecorder } from '../../../core/recorder.ts';
import { slrParse, buildTable, makeSampleGrammar, type Grammar, type SlrHooks } from './impl.ts';
...
export function buildTrace(input: string[] = DEFAULT_INPUT): Frame[] {
  ...
  const { sets, table, follow } = buildTable(g);
  ...
  const followAux = [...follow.entries()].map(([nt, s]) => ({
    label: `FOLLOW(${nt})`,
    value: `{${[...s].join(',')}}`,
  ...
  rec
    .begin({
      zh: `SLR(1) 文法（增广 E'→E）。输入：${input.join(' ')} $`,
      en: `SLR(1) grammar (augmented E'→E). Input: ${input.join(' ')} $`,
    })
    .setAux([...grammarAux, ...followAux])
    .commit();
  ...
  rec
    .begin({
      zh: `LR(0) 项目集规范族：${sets.length} 个状态`,
      en: `LR(0) canonical item sets: ${sets.length} states`,
    })
    .setGrid(itemGrid)
  ...
  const hooks: SlrHooks = {
    ...
    onReduce: (prodIndex, lhs, rhsLen) => {
      const prod = g.productions[prodIndex]!;
      rec
        .begin({
          zh: `归约：用产生式 (${prodIndex}) ${lhs} → ${prod.rhs.length === 0 ? 'ε' : prod.rhs.join(' ')}（弹 ${rhsLen} 项）`,
          en: `Reduce: by production (${prodIndex}) ${lhs} → ${prod.rhs.length === 0 ? 'ε' : prod.rhs.join(' ')} (pop ${rhsLen})`,
        })
        .setGrid(tableGrid)
        .setAux([
    ...
    onAccept: () => {
      rec
        .begin({
          zh: `接受：输入符合文法`,
          en: `Accept: input matches the grammar`,
        })
        .setGrid(tableGrid)
    ...
    onError: (token, state) => {
      rec
        .begin({
          zh: `报错：状态 ${state} 遇到 token "${token}" 无动作`,
          en: `Error: no action in state ${state} for token "${token}"`,
        })
        .setGrid(tableGrid)
  ...
  const result = slrParse(input, g, hooks);
