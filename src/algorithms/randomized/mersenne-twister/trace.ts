// =============================================================================
// 梅森旋转 MT19937 · 录制帧序列
// 用 bars 展示生成的无符号 32 位值（归一化柱高），aux 展示序列与周期信息。
// =============================================================================

import type { BarRole, Frame } from '../../../types.ts';
import { TraceRecorder } from '../../../core/recorder.ts';
import { MersenneTwister, type MersenneTwisterHooks } from './impl.ts';

export const DEFAULT_INPUT = { seed: 542171377, count: 16 };
...
export function buildTrace(input: BuildTraceInput = {}): Frame[] {
  ...
  const render = (note: { zh: string; en: string }): void => {
    const aux: Array<{ label: string; value: string; role?: BarRole }> = [
      ...
      { label: '状态字', value: '624', role: 'pivot' },
      { label: '周期', value: '2¹⁹⁹³⁷−1', role: 'pivot' },
  ...
  render({
    zh: `MT19937 种子 ${seed}，生成 ${count} 个无符号 32 位数`,
    en: `MT19937 seed ${seed}, generate ${count} unsigned 32-bit numbers`,
  });

  const hooks: MersenneTwisterHooks = {
    onNext: (v) => {
      seq.push(v);
      render({
        zh: `第 ${seq.length} 个：${v}`,
