...
export function buildTrace(input: number[] = DEFAULT_INPUT, k: number = DEFAULT_K): Frame[] {
  ...
  const hooks: QuickselectHooks = {
    onPartition: (lo, hi, rank, p) => {
      pivotIdx = p;
      snapshot({
        zh: `划分 [${lo}, ${hi}]（目标 rank=${rank}），基准 = ${a[p]}（下标 ${p}）`,
        en: `Partition [${lo}, ${hi}] (target rank=${rank}), pivot = ${a[p]} (idx ${p})`,
      });
    },
    onCompare: (i) => {
      comparing.add(i);
