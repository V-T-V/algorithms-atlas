// =============================================================================
// 合并有序链表（哑节点变种）· 录制帧序列
// =============================================================================

import type { Frame } from '../../../types.ts';
import { TraceRecorder } from '../../../core/recorder.ts';
import { buildList, listToArray, mergeSortedAlt, type MergeSortedAltHooks } from './impl.ts';

export const DEFAULT_INPUT: { a: number[]; b: number[] } = { a: [1, 2, 4], b: [1, 3, 4] };

export function buildTrace(input: { a: number[]; b: number[] } = DEFAULT_INPUT): Frame[] {
  const rec = new TraceRecorder();
  const { a, b } = input;
  const headA = buildList(a);
  const headB = buildList(b);
  const out: number[] = [];
  let lastPick: 'a' | 'b' = 'a';

  const snap = (note: { zh: string; en: string }): void => {
    rec
      .begin(note)
      .setAux([
        { label: 'a', value: a.join(' → ') },
        { label: 'b', value: b.join(' → ') },
        { label: 'out', value: out.join(' → ') || '-', role: 'frontier' },
        { label: 'lastPick', value: lastPick },
      ])
      .commit();
  };

  snap({ zh: '合并两条升序链表', en: 'Merge two sorted lists' });

  const hooks: MergeSortedAltHooks = {
    onPick: (value, from) => {
      out.push(value);
      lastPick = from;
      snap({
        zh: `选取 ${value}（来自 ${from}）`,
        en: `Pick ${value} (from ${from})`,
      });
    },
  };

  const merged = mergeSortedAlt(headA, headB, hooks);
  void merged;

  snap({ zh: `完成：${out.join(' → ')}`, en: `Done: ${out.join(' → ')}` });
  return rec.build();
}
