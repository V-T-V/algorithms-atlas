...
import { integrateLeapfrog } from './impl.ts';

export const DEFAULT_INPUT = {
  // 简谐振子 x'' = -x，初值 x=1, v=0，周期 2π
...
export function buildTrace(
  ...
  rec
    .begin({
      zh: `Leapfrog：[ ${t0.toFixed(3)}, ${t1.toFixed(3)} ]，${steps} 步`,
      en: `Leapfrog: [${t0.toFixed(3)}, ${t1.toFixed(3)}], ${steps} steps`,
    })
    .setAux([
  ...
  const trj = integrateLeapfrog(accel, t0, x0, v0, t1, steps, {
    onStep: (t, x, v) => {
      // 能量 E = 0.5 v² + 0.5 x²（简谐振子）
