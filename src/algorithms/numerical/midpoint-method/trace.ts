// =============================================================================
// 中点法 · 录制帧序列
// 解 y' = y（精确解 e^x），setBars 展示 y 序列 + setAux 展示 k1/k2。
// =============================================================================

import type { BarRole, Frame } from '../../../types.ts';
import { TraceRecorder } from '../../../core/recorder.ts';
import { midpointMethod, type MidpointHooks } from './impl.ts';
...
export function buildTrace(input?: {
  ...
  const snapshot = (note: { zh: string; en: string }, ks?: [number, number]): void => {
    ...
    if (ks) {
      aux.push(
        ...
        { label: 'k2（中点）', value: ks[1]!.toFixed(4), role: 'compare' as BarRole },
      );
    }
    rec
      .begin(note)
  ...
  const hooks: MidpointHooks = {
    onStep: (step, x, y, k1, k2) => {
      ys.push(y);
      snapshot(
        {
  ...
  void midpointMethod(f, x0, y0, xEnd, h, hooks);
