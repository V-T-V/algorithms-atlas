// opt-tf-adam · 实现（TensorFlow Adam 默认参数）
export interface TfAdamHooks {
  onIter?: (iter: number, params: number[], value: number) => void;
}
export interface TfAdamResult {
  params: number[];
  value: number;
  iterations: number;
  converged: boolean;
}
export function optTfAdam(
  f: (p: number[]) => number,
  ...
  const { lr = 0.5, beta1 = 0.9, beta2 = 0.999, eps = 1e-7, maxIter = 500, tol = 1e-8 } = opts;
  const params = [...init];
