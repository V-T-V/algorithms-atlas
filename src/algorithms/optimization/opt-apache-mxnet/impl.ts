// opt-apache-mxnet · 实现（MXNet Adam 默认参数）
export interface ApacheMxnetHooks {
  onIter?: (iter: number, params: number[], value: number) => void;
}
export interface ApacheMxnetResult {
  params: number[];
  value: number;
  iterations: number;
  converged: boolean;
}
export function optApacheMxnet(
  f: (p: number[]) => number,
  ...
  hooks: ApacheMxnetHooks = {},
): ApacheMxnetResult {
  const { lr = 0.5, beta1 = 0.9, beta2 = 0.999, eps = 1e-8, maxIter = 500, tol = 1e-8 } = opts;
  const params = [...init];
