// AdaGrad · 录制帧序列
// 用 setAux 展示参数 / 梯度 / 累积平方梯度 G / 目标值随迭代变化。

import type { BarRole, Frame } from '../../../types.ts';
import { TraceRecorder } from '../../../core/recorder.ts';
import { adaGrad, demoFunc, demoGrad, type AdaGradHooks } from './impl.ts';

export const DEFAULT_INPUT = { initParams: [0, 0], lr: 1.0, maxIter: 100, tol: 1e-10 };

export function buildTrace(
  input: { initParams?: number[]; lr?: number; maxIter?: number; tol?: number } = {},
): Frame[] {
  const { initParams = [0, 0], lr = 1.0, maxIter = 100, tol = 1e-10 } = input;
  ...
  const snapshot = (
    ...
    rec
      ...
      .setAux([
        ...
        { label: 'G_x', value: accSq[0]!.toFixed(4), role: 'default' as BarRole },
        { label: 'G_y', value: accSq[1]!.toFixed(4), role: 'default' as BarRole },
  ...
  const hooks: AdaGradHooks = {
    onIter: (iter, params, grad, accSq, value) => {
      snapshot(
        {
          zh: `迭代 ${iter}：f=${value.toFixed(6)}，自适应学习率使步长逐步缩小`,
          en: `Iter ${iter}: f=${value.toFixed(6)}, adaptive LR shrinks steps`,
        },
        params,
        grad,
        accSq,
  ...
  const result = adaGrad(demoFunc, demoGrad, initParams, { lr, maxIter, tol }, hooks);
