...
import { dfp, type DFPHooks } from './impl.ts';

export const DEFAULT_INPUT = {
  f: (x: number[]): number => (x[0]! - 3) ** 2 + (x[1]! + 1) ** 2,
  g: (x: number[]): number[] => [2 * (x[0]! - 3), 2 * (x[1]! + 1)],
  x0: [0, 0],
};
...
export function buildTrace(input: typeof DEFAULT_INPUT = DEFAULT_INPUT): Frame[] {
  ...
  rec
    .begin({
      zh: `DFP 最小化目标函数，初始 (${x0.join(',')})，目标点 (3,-1)。`,
      en: `DFP minimizing objective, init (${x0.join(',')}), target (3,-1).`,
    })
    .setAux([
  ...
  const hooks: DFPHooks = {
    ...
    onUpdate: (s, y) => {
      rec
        .begin({
          zh: `DFP 秩二更新：s=(${s.map((v) => v.toFixed(3)).join(',')}), y=(${y.map((v) => v.toFixed(3)).join(',')})`,
          en: `DFP rank-2 update: s=(${s.map((v) => v.toFixed(3)).join(',')}), y=(${y.map((v) => v.toFixed(3)).join(',')})`,
        })
        .setAux([
          {
            label: 's',
            value: `(${s.map((v) => v.toFixed(3)).join(',')})`,
            role: 'compare' as BarRole,
          },
          {
            label: 'y',
            value: `(${y.map((v) => v.toFixed(3)).join(',')})`,
  ...
  const result = dfp(f, g, x0, { maxIter: 100, tol: 1e-10 }, hooks);
