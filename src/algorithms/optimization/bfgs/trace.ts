...
import { bfgs, demoFunc, demoGrad, type BFGSHooks } from './impl.ts';
...
export function buildTrace(
  ...
  snapshot(
    { zh: '初始 (0,0)，海森逆近似 H=I', en: 'Init (0,0), Hessian-inverse approx H=I' },
    initParams,
    demoGrad(initParams),
    demoFunc(initParams),
    0,
  ...
  const hooks: BFGSHooks = {
    onIter: (iter, params, grad, value) => {
      snapshot(
        {
          zh: `迭代 ${iter}：f=${value.toFixed(6)}，用梯度差更新海森逆近似`,
          en: `Iter ${iter}: f=${value.toFixed(6)}, update H⁻¹ approx via gradient diff`,
        },
        params,
        grad,
        value,
  ...
  const result = bfgs(demoFunc, demoGrad, initParams, { maxIter, tol }, hooks);
