// 梯度下降 · 录制帧序列
// 用 setAux 展示参数/梯度/目标值随迭代变化。

import type { BarRole, Frame } from '../../../types.ts';
import { TraceRecorder } from '../../../core/recorder.ts';
import { gradientDescent, demoFunc, demoGrad, type GradientDescentHooks } from './impl.ts';

export const DEFAULT_INPUT = { initParams: [0, 0], lr: 0.1, maxIter: 50, tol: 1e-6 };

export function buildTrace(
  input: { initParams?: number[]; lr?: number; maxIter?: number; tol?: number } = {},
): Frame[] {
  const { initParams = [0, 0], lr = 0.1, maxIter = 50, tol = 1e-6 } = input;
  ...
  snapshot(
    { zh: '初始参数 (0,0)，目标最小化 (x-3)²+(y+1)²', en: 'Init (0,0), minimize (x-3)²+(y+1)²' },
    initParams,
    demoGrad(initParams),
    demoFunc(initParams),
    0,
  ...
  const hooks: GradientDescentHooks = {
    onIter: (iter, params, grad, value) => {
      snapshot(
        {
          zh: `迭代 ${iter}：f=${value.toFixed(6)}，向 (3,-1) 靠近`,
          en: `Iter ${iter}: f=${value.toFixed(6)}, approaching (3,-1)`,
        },
        params,
        grad,
        value,
  ...
  const result = gradientDescent(demoFunc, demoGrad, initParams, lr, maxIter, tol, hooks);
