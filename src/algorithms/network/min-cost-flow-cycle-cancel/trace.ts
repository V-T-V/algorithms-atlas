...
import { cycleCancel, type CcEdgeInput, type CcHooks } from './impl.ts';
...
export function buildTrace(
  input: { n: number; edges: CcEdgeInput[]; s: number; t: number } = DEFAULT_INPUT,
): Frame[] {
  ...
  const render = (note: { zh: string; en: string }, curFlow = 0, curCost = 0): void => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (cycleNodes.has(i)) role = 'compare';
      nodes.push({
        id: String(i),
        label: String(i),
        x: POS[i]?.x ?? 0.5,
    ...
    const e2: GraphEdge[] = edges.map((e) => ({
      ...
      weight: e.cost,
      directed: true,
      role: ((flow.get(`${e.from}>${e.to}`) ?? 0) > 0 ? 'frontier' : 'default') as BarRole,
    }));
    const aux = [
  ...
  const hooks: CcHooks = {
    onMaxFlowFound: (maxFlow, initialCost) => {
      step += 1;
      curMaxFlow = maxFlow;
      render(
        {
          zh: `阶段 1 完成：最大流 = ${maxFlow}，初始费用 = ${initialCost}`,
          en: `Phase 1 done: max flow = ${maxFlow}, initial cost = ${initialCost}`,
        },
        maxFlow,
        initialCost,
      );
    },
    onNegativeCycle: (cycle, cycleCost, pushed, totalCostAfter) => {
      step += 1;
      cycle.forEach((c) => cycleNodes.add(c));
      render(
        {
          zh: `消负环 [${cycle.join('→')}]：环费 ${cycleCost}，推 ${pushed}，新费用 ${totalCostAfter}`,
          en: `Cancel neg-cycle [${cycle.join('→')}]: cost ${cycleCost}, push ${pushed}, new cost ${totalCostAfter}`,
        },
        curMaxFlow,
        totalCostAfter,
      );
  ...
  const result = cycleCancel(n, edges, s, t, hooks);
