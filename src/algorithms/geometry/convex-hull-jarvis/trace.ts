...
import { convexHullJarvis, type Point, type ConvexHullJarvisHooks } from './impl.ts';
...
export function buildTrace(input: HullInput = DEFAULT_INPUT): Frame[] {
  ...
  const hooks: ConvexHullJarvisHooks = {
    onStart: (p) => {
      rec
        .begin({ zh: `起点（最左下）`, en: `Start (leftmost-bottom)` })
        .setGraph(
          points.map((q, i) => ({
            id: `p${i}`,
            label: String(i),
            x: px(q.x),
            y: py(q.y),
            role: (q.x === p.x && q.y === p.y ? 'pivot' : 'default') as BarRole,
          })),
          [],
        )
        .commit();
  ...
  const { hull } = convexHullJarvis(points, hooks);
