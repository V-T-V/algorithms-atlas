...
import { horspool2, type Horspool2Hooks } from './impl.ts';
...
export function buildTrace(input: { text: string; pat: string } = DEFAULT_INPUT): Frame[] {
  ...
  const aux = (extra: string): Array<{ label: string; value: string; role?: BarRole }> => [
    { label: 'text', value: text },
    { label: 'pat', value: pat },
    { label: 'pos', value: pos < 0 ? '-' : String(pos), role: 'frontier' },
    { label: 'j', value: j < 0 ? '-' : String(j), role: 'compare' },
    { label: 'note', value: extra },
  ];
  ...
  const hooks: Horspool2Hooks = {
    ...
    onShift: (from, to, badChar) => {
      pos = to;
      roleTip = 'frontier';
      snap({
        zh: `按坏字符 '${badChar}' 查表，pos 从 ${from} 滑到 ${to}`,
        en: `By bad char '${badChar}', pos shifts ${from} -> ${to}`,
      });
    },
    onFound: (start) => {
      matches.push(start);
