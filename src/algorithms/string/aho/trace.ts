// =============================================================================
// Aho 搜索（单模式自动机）· 录制帧序列
// setArray 展示主串（字符码），pointer 标注当前 i；setAux 展示当前状态 q。
// =============================================================================

import type { BarRole, Frame } from '../../../types.ts';
import { TraceRecorder } from '../../../core/recorder.ts';
import { aho, type AhoHooks } from './impl.ts';
...
export function buildTrace(input: { text: string; pat: string } = DEFAULT_INPUT): Frame[] {
  ...
  snap({ zh: `Aho 自动机搜索：${pat}`, en: `Aho automaton search: ${pat}` });

  const hooks: AhoHooks = {
    onBuildTrans: () => {},
    onTransfer: (idx, q, nq) => {
      i = idx;
      curQ = nq;
