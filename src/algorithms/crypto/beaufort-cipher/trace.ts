...
import { beaufort, type BeaufortHooks } from './impl.ts';

export const DEFAULT_INPUT = { text: 'HELLO WORLD', key: 'KEY' };
...
export function buildTrace(input: { text: string; key: string } = DEFAULT_INPUT): Frame[] {
  ...
  snapshot({ zh: `明文「${text}」，密钥「${key}」`, en: `Plaintext "${text}", key "${key}"` });

  const hooks: BeaufortHooks = {
    onMap: (i, plain, keyChar, cipher) => {
      cursor = i;
      display[i] = cipher.charCodeAt(0);
      snapshot({
        zh: `'${plain}' ${cipher === plain ? '==' : '→'} '${cipher}'（密钥 '${keyChar}'）`,
  ...
  const { text: out } = beaufort(text, key, hooks);
