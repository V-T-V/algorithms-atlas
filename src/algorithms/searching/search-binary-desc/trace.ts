...
import { binarySearchDesc, type BinDescHooks } from './impl.ts';

export const DEFAULT_INPUT = [21, 19, 17, 15, 13, 11, 9, 7, 5, 3, 1];
...
export function buildTrace(
  ...
  const result = binarySearchDesc(input, target, hooks);
  const roles: BarRole[] = new Array(n).fill('default');
