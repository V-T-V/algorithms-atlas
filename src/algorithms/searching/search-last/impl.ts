// =============================================================================
// 查找最后一个等于（Upper Bound - 1）· 纯算法实现
// 在升序数组中找最后一个等于 target 的下标（含重复元素）。
// 零 DOM 依赖，可独立单测。
// =============================================================================

/** 算法执行过程中的事件钩子。任一可选；实现录制器按需实现。 */
export interface SearchLastHooks {
  /** 在 [lo,hi] 区间探测中点 mid。 */
...
/**
 * 找最后一个等于 target 的下标；不存在返回 -1。
 */
export function searchLast(
  arr: readonly number[],
  target: number,
  hooks: SearchLastHooks = {},
): number {
  let lo = 0;
  let hi = arr.length - 1;
  let result = -1;
