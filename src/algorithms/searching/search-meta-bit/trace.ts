...
import { metaBitSearch, type MetaBitHooks } from './impl.ts';
...
export function buildTrace(
  ...
  rec
    ...
    .commit();
  const hooks: MetaBitHooks = {
    onTry: (i: number) => {
      const roles: BarRole[] = new Array(n).fill('default');
      roles[i] = 'compare';
      rec
        .begin({ zh: `尝试 a[${i}]=${values[i]}`, en: `尝试 a[${i}]=${values[i]}` })
        .setArray(values, roles, [{ index: i, label: 'i' }])
        .commit();
    },
  };
  const result = metaBitSearch(input, target, hooks);
  const roles: BarRole[] = new Array(n).fill('default');
