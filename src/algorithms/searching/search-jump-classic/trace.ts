...
import { jumpSearchClassic, type JumpClassicHooks } from './impl.ts';
...
export function buildTrace(
  ...
  rec
    ...
    .commit();
  const hooks: JumpClassicHooks = {
    onJump: (i: number) => {
      const roles: BarRole[] = new Array(n).fill('default');
      roles[i] = 'compare';
      rec
        .begin({ zh: `跳跃 a[${i}]=${values[i]}`, en: `跳跃 a[${i}]=${values[i]}` })
        .setArray(values, roles, [{ index: i, label: 'i' }])
        .commit();
    },
  };
  const result = jumpSearchClassic(input, target, hooks);
  const roles: BarRole[] = new Array(n).fill('default');
