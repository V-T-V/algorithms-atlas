...
import { gallopSearch3, type Gallop3Hooks } from './impl.ts';
...
export function buildTrace(
  ...
  rec
    ...
    .commit();
  const hooks: Gallop3Hooks = {
    onGallop: (i: number) => {
      const roles: BarRole[] = new Array(n).fill('default');
      roles[i] = 'compare';
      rec
        .begin({ zh: `飞奔 a[${i}]=${values[i]}`, en: `飞奔 a[${i}]=${values[i]}` })
        .setArray(values, roles, [{ index: i, label: 'i' }])
        .commit();
    },
  };
  const result = gallopSearch3(input, target, hooks);
  const roles: BarRole[] = new Array(n).fill('default');
