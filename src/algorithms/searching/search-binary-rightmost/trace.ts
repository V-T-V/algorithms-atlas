...
import { binarySearchRightmost, type BinRightHooks } from './impl.ts';

export const DEFAULT_INPUT = [1, 3, 3, 3, 5, 7, 9, 11, 11, 13];
...
export function buildTrace(
  ...
  const result = binarySearchRightmost(input, target, hooks);
  const roles: BarRole[] = new Array(n).fill('default');
