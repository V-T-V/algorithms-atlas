// =============================================================================
// 二分查找 · 录制帧序列
// 通过 binarySearch 的钩子，把执行过程录成 Frame[]。
// =============================================================================

import type { BarRole, Frame } from '../../../types.ts';
import { TraceRecorder } from '../../../core/recorder.ts';
import { binarySearch, type BinarySearchHooks } from './impl.ts';

export const DEFAULT_INPUT = [1, 3, 5, 7, 9, 11, 13, 15, 17];
/** 演示默认要查找的目标值。 */
export const DEFAULT_TARGET = 7;

/** 录制演示帧序列。input 须为升序数组。 */
export function buildTrace(
  input: number[] = DEFAULT_INPUT,
  target: number = DEFAULT_TARGET,
): Frame[] {
  const rec = new TraceRecorder();
  const values = [...input];
  let lo = 0;
  let hi = values.length - 1;
  let mid = -1;
  let outOfRange = false; // 区间为空

  const rolesArr = (): BarRole[] => {
    const roles: BarRole[] = new Array(values.length).fill('default');
    // 区间内的元素标记为边界
    if (!outOfRange) {
      for (let k = lo; k <= hi; k++) roles[k] = 'frontier';
    }
    if (mid >= 0 && mid >= lo && mid <= hi) roles[mid] = 'pivot';
    return roles;
  };

  const snapshot = (
    note: { zh: string; en: string },
    pointers: Array<{ index: number; label: string }>,
  ): void => {
    rec.begin(note).setArray(values, rolesArr(), pointers).commit();
  };

  snapshot(
    {
      zh: `升序数组中查找 ${target}，区间 [${lo}, ${hi}]`,
      en: `Search ${target} in a sorted array, range [${lo}, ${hi}]`,
    },
    [
      { index: lo, label: 'lo' },
      { index: hi, label: 'hi' },
    ],
  );

  const hooks: BinarySearchHooks = {
    onProbe: (curLo, curHi, curMid) => {
      lo = curLo;
      hi = curHi;
      mid = curMid;
      snapshot(
        {
          zh: `中点 mid=${mid}，a[${mid}]=${values[mid]} 与目标 ${target} 比较`,
          en: `Mid=${mid}, a[${mid}]=${values[mid]} vs target ${target}`,
        },
        [
          { index: lo, label: 'lo' },
          { index: mid, label: 'mid' },
          { index: hi, label: 'hi' },
        ],
      );
    },
    onShrink: (newLo, newHi, dir) => {
      lo = newLo;
      hi = newHi;
      if (lo > hi) outOfRange = true;
      const tip =
        dir === 'right'
          ? {
              zh: `目标更大 → 去右半 [${lo}, ${hi}]`,
              en: `Target is larger → right half [${lo}, ${hi}]`,
            }
          : {
              zh: `目标更小 → 去左半 [${lo}, ${hi}]`,
              en: `Target is smaller → left half [${lo}, ${hi}]`,
            };
      snapshot(tip, [
        { index: Math.max(lo, 0), label: 'lo' },
        { index: Math.min(hi, values.length - 1), label: 'hi' },
      ]);
    },
    onDone: (found) => {
      const roles: BarRole[] = new Array(values.length).fill('default');
      if (found >= 0) roles[found] = 'final';
      rec
        .begin(
          found >= 0
            ? { zh: `命中：下标 ${found}`, en: `Found at index ${found}` }
            : { zh: `未找到 ${target}`, en: `${target} not found` },
        )
        .setArray(values, roles, found >= 0 ? [{ index: found, label: '✓' }] : [])
        .commit();
    },
  };

  binarySearch(input, target, hooks);

  return rec.build();
}
