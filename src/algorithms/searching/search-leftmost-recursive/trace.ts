...
import { binarySearchLeftmostRecursive, type LeftRecurHooks } from './impl.ts';

export const DEFAULT_INPUT = [1, 3, 3, 3, 5, 7, 9];
...
export function buildTrace(
  ...
  const result = binarySearchLeftmostRecursive(input, target, hooks);
  const roles: BarRole[] = new Array(n).fill('default');
