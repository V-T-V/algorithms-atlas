...
import { searchFirst, type SearchFirstHooks } from './impl.ts';
...
export function buildTrace(
  ...
  snap({ zh: `查找第一个 ${target}`, en: `Find first ${target}` });

  const hooks: SearchFirstHooks = {
    ...
    onDone: (idx) => {
      ...
      rec
        .begin(
          idx >= 0
            ? { zh: `首个 ${target} 在 ${idx}`, en: `First ${target} at ${idx}` }
            : { zh: '未找到', en: 'Not found' },
        )
  ...
  searchFirst(input, target, hooks);
  return rec.build();
}
