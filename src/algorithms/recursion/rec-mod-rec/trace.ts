...
import { recModRec } from './impl.ts';

export const DEFAULT_INPUT = { a: 17, b: 5 };

export function buildTrace(input: typeof DEFAULT_INPUT = DEFAULT_INPUT): Frame[] {
  const rec = new TraceRecorder();
  rec
    .begin({ zh: 'rec-mod-rec：开始递归', en: 'rec-mod-rec: start recursion' })
  ...
  const r = recModRec(input.a, input.b, {
    onRecurse: (d, x, y) =>
      events.push({
        note: { zh: `mod(${x},${y})`, en: `mod(${x},${y})` },
