// rec-partition-rec · 录制
import type { BarRole, Frame } from '../../../types.ts';
import { TraceRecorder } from '../../../core/recorder.ts';
import { recPartitionRec } from './impl.ts';
...
export function buildTrace(input: typeof DEFAULT_INPUT = DEFAULT_INPUT): Frame[] {
  ...
  rec
    .begin({ zh: 'rec-partition-rec：开始递归', en: 'rec-partition-rec: start recursion' })
  ...
  const r = recPartitionRec(input.n, input.k, {
    onRecurse: (d, a, b) => {
      if (events.length < 10)
        events.push({
