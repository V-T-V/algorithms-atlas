// rec-combination-rec · 录制
import type { BarRole, Frame } from '../../../types.ts';
import { TraceRecorder } from '../../../core/recorder.ts';
import { recCombinationRec } from './impl.ts';
...
export function buildTrace(input: typeof DEFAULT_INPUT = DEFAULT_INPUT): Frame[] {
  ...
  rec
    .begin({ zh: 'rec-combination-rec：开始递归', en: 'rec-combination-rec: start recursion' })
  ...
  const r = recCombinationRec(input.n, input.k, {
    onRecurse: (d, a, b) => {
      if (events.length < 10)
        events.push({
          note: { zh: `递归 C(${a},${b})`, en: `recurse C(${a},${b})` },
