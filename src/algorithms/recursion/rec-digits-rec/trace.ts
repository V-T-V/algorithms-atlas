...
import { recDigitsRec } from './impl.ts';

export const DEFAULT_INPUT = { n: 12345 };

export function buildTrace(input: typeof DEFAULT_INPUT = DEFAULT_INPUT): Frame[] {
  const rec = new TraceRecorder();
  rec
    .begin({ zh: 'rec-digits-rec：开始递归', en: 'rec-digits-rec: start recursion' })
  ...
  const r = recDigitsRec(input.n, {
    onRecurse: (d, k) =>
      events.push({
        note: { zh: `sumDigits(${k})`, en: `sumDigits(${k})` },
