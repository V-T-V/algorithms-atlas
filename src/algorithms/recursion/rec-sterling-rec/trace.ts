...
import { recSterlingRec } from './impl.ts';
...
export function buildTrace(input: typeof DEFAULT_INPUT = DEFAULT_INPUT): Frame[] {
  ...
  rec
    .begin({ zh: 'rec-sterling-rec：开始递归', en: 'rec-sterling-rec: start recursion' })
  ...
  const r = recSterlingRec(input.n, input.k, {
    onRecurse: (d, a, b) => {
      if (events.length < 10)
        events.push({
          note: { zh: `递归 S(${a},${b})`, en: `recurse S(${a},${b})` },
