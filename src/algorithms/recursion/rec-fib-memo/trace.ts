...
import { recFibMemo } from './impl.ts';
...
export function buildTrace(input: typeof DEFAULT_INPUT = DEFAULT_INPUT): Frame[] {
  ...
  rec
    .begin({ zh: 'rec-fib-memo：开始递归', en: 'rec-fib-memo: start recursion' })
  ...
  const r = recFibMemo(input.n, {
    ...
    onCache: (k, v) =>
      events.push({
        note: { zh: `缓存命中 fib(${k})=${v}`, en: `cache hit fib(${k})=${v}` },
        aux: [{ label: 'hit', value: String(k), role: 'compare' as BarRole }],
      }),
    onReturn: (d, k, v) => {
      if (events.length < 12)
        events.push({
          note: { zh: `返回 fib(${k})=${v}`, en: `return fib(${k})=${v}` },
