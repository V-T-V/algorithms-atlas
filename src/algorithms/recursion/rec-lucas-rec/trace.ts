...
import { recLucasRec } from './impl.ts';
...
export function buildTrace(input: typeof DEFAULT_INPUT = DEFAULT_INPUT): Frame[] {
  ...
  rec
    .begin({ zh: 'rec-lucas-rec：开始递归', en: 'rec-lucas-rec: start recursion' })
  ...
  const r = recLucasRec(input.n, {
    onRecurse: (d, k) => {
      if (events.length < 10)
        events.push({
          note: { zh: `递归 L(${k})`, en: `recurse L(${k})` },
