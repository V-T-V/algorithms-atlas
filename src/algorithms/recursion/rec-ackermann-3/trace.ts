...
import { recAckermann3 } from './impl.ts';
...
export function buildTrace(input: typeof DEFAULT_INPUT = DEFAULT_INPUT): Frame[] {
  ...
  rec
    .begin({ zh: 'rec-ackermann-3：开始递归', en: 'rec-ackermann-3: start recursion' })
  ...
  const r = recAckermann3(input.m, input.n, {
    onRecurse: (d, a, b) => {
      if (events.length < 10)
        events.push({
          note: { zh: `递归 A(${a},${b})`, en: `recurse A(${a},${b})` },
