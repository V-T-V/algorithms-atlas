...
import { recMultiplyRec } from './impl.ts';

export const DEFAULT_INPUT = { a: 6, b: 7 };

export function buildTrace(input: typeof DEFAULT_INPUT = DEFAULT_INPUT): Frame[] {
  const rec = new TraceRecorder();
  rec
    .begin({ zh: 'rec-multiply-rec：开始递归', en: 'rec-multiply-rec: start recursion' })
  ...
  const r = recMultiplyRec(input.a, input.b, {
    onRecurse: (d, x, y) =>
      events.push({
    ...
    onBase: (d) =>
      events.push({ note: { zh: `基线 d=${d}: 返回 0`, en: `base d=${d}: return 0` }, aux: [] }),
    onReturn: (d, v) =>
      events.push({
