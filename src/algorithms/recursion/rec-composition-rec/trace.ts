// rec-composition-rec · 录制
import type { BarRole, Frame } from '../../../types.ts';
import { TraceRecorder } from '../../../core/recorder.ts';
import { recCompositionRec } from './impl.ts';
...
export function buildTrace(input: typeof DEFAULT_INPUT = DEFAULT_INPUT): Frame[] {
  ...
  rec
    .begin({ zh: 'rec-composition-rec：开始递归', en: 'rec-composition-rec: start recursion' })
  ...
  const r = recCompositionRec(input.n, input.k, {
    onRecurse: (d, a, b) => {
      if (events.length < 10)
        events.push({
          note: { zh: `递归 comp(${a},${b})`, en: `recurse comp(${a},${b})` },
