...
import { modularPower, type ModularPowerHooks } from './impl.ts';
...
export function buildTrace(
  ...
  let curBase = base;
  let curExp = exp;
  let lastAction: 'square' | 'multiply' | 'bit' | null = null;
  ...
  const auxRows = (note: { zh: string; en: string }): void => {
    ...
    rec
      ...
      .setAux([
        ...
        { label: 'mod', value: String(mod), role: 'default' },
      ])
      .commit();
    lastAction = null;
  };
  ...
  auxRows({
    zh: `计算 ${base}^${exp} mod ${mod}（指数二进制 = ${toBin(exp)}）`,
    en: `Compute ${base}^${exp} mod ${mod} (exp in binary = ${toBin(exp)})`,
  });

  const hooks: ModularPowerHooks = {
    onBit: (bit) => {
      lastAction = 'bit';
      auxRows({
        zh: `看指数最低位 = ${bit}${bit === 1 ? '（累乘 result *= base）' : '（跳过）'}`,
    ...
    onMultiply: (r) => {
      result = r;
      lastAction = 'multiply';
      auxRows({ zh: `该位为 1：result = ${r}`, en: `Bit is 1: result = ${r}` });
    },
    onSquare: (nb) => {
      curBase = nb;
      curExp = Math.floor(curExp / 2);
      lastAction = 'square';
      auxRows({ zh: `base 自乘平方 → ${nb}；指数右移`, en: `Square base → ${nb}; shift exp` });
    },
  };

  const ans = modularPower(base, exp, mod, hooks);

  rec
    .begin({
      zh: `结果：${base}^${exp} mod ${mod} = ${ans}`,
      en: `Result: ${base}^${exp} mod ${mod} = ${ans}`,
    })
    .setAux([
