// =============================================================================
// 双端队列 · 录制帧序列
// 用 setArray 展示环形缓冲区，pointer 标 head/tail；setAux 展示逻辑序列。
// =============================================================================
...
export function buildTrace(
  ...
  const snapshot = (
    ...
    for (let k = 0; k < q.size; k++) {
      const idx = (q.head + k) % q.capacity;
      roles[idx] = 'frontier';
    }
    if (activeIdx >= 0) roles[activeIdx] = activeRole;
    const pointers: Array<{ index: number; label: string }> = [];
    if (!q.isEmpty()) {
      pointers.push({ index: q.head, label: 'head' });
      pointers.push({ index: q.tail, label: 'tail' });
    } else {
      pointers.push({ index: q.tail, label: 'head=tail' });
    }
    rec
      .begin(note)
      ...
      .setAux([
        {
          label: '逻辑序列 (front→back)',
          value: `[${q.toSequence().join(', ')}]`,
          role: 'final' as BarRole,
        },
        { label: 'size / capacity', value: `${q.size} / ${q.capacity}` },
      ])
      .commit();
  };
  ...
  const hooks: DequeHooks = {
    ...
    onGrow: (oldCap, newCap) => {
      snapshot(
        {
          zh: `扩容 ${oldCap} → ${newCap}，元素搬移到 [0, ${q.size})`,
          en: `Grow ${oldCap} → ${newCap}, reflow to [0, ${q.size})`,
        },
        -1,
        'warn',
      );
  ...
  rec
    ...
    .setArray(
      ...
      q.toArray().map((_, i) => {
        let used = false;
        for (let k = 0; k < q.size; k++) if ((q.head + k) % q.capacity === i) used = true;
        return used ? ('final' as BarRole) : ('default' as BarRole);
      }),
      q.isEmpty()
        ? [{ index: q.tail, label: 'head=tail' }]
        : [
            { index: q.head, label: 'head' },
            { index: q.tail, label: 'tail' },
          ],
    )
    .setAux([
      { label: '逻辑序列 (front→back)', value: `[${seq.join(', ')}]`, role: 'final' as BarRole },
      { label: 'size / capacity', value: `${q.size} / ${q.capacity}` },
    ])
    .commit();
