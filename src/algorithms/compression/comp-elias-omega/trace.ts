...
import { eliasOmegaEncodeAll, eliasOmegaDecodeAll, type EliasOmegaHooks } from './impl.ts';
...
export function buildTrace(input: readonly number[] = DEFAULT_INPUT): Frame[] {
  ...
  const hooks: EliasOmegaHooks = {
    onEncode: (n, bits) => codes.push({ n, bits }),
  };

  const bitstream = eliasOmegaEncodeAll(input, hooks);
  const decoded = eliasOmegaDecodeAll(bitstream);
  ...
  rec
    .begin({
      ...
      en: `Done: ${bitstream.length} bits, roundtrip ${ok ? 'OK' : 'FAIL'}`,
    })
    .setAux([
