...
import { sarsa } from './impl.ts';
import { qToPolicy } from '../ais-q-learning-simple/impl.ts';
import type { QDomain } from '../ais-q-learning-simple/impl.ts';
...
export const DEFAULT_INPUT = { gamma: 0.9, alpha: 0.3, epsilon: 0.2, episodes: 200 };
...
export function buildTrace(input = DEFAULT_INPUT): Frame[] {
  ...
  rec
    .begin({ zh: 'SARSA：Q=0', en: 'SARSA: Q=0' })
    ...
    hooks: {
      onEpisode: (ep, _ret, Qcur) => {
        ...
        if (snap % 40 === 0 || ep === episodes - 1) {
          rec
            .begin({ zh: `第 ${ep + 1} 回合`, en: `Episode ${ep + 1}` })
            .setBars(
  ...
  rec
    .begin({ zh: '收敛 SARSA Q 表', en: 'Converged SARSA Q-table' })
    .setBars(
