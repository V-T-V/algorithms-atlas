// =============================================================================
// 强连通分量·递归 Tarjan · 录制帧序列
import type { BarRole, Frame, GraphEdge, GraphNode } from '../../../types.ts';
import { TraceRecorder } from '../../../core/recorder.ts';
import { sccTarjanRecursive, type GraphInput, type SccTarjanRecHooks } from './impl.ts';
...
export function buildTrace(input: GraphInput = DEFAULT_INPUT): Frame[] {
  ...
  render({ zh: '初始有向图（递归 Tarjan）', en: 'Initial directed graph (recursive Tarjan)' });

  const hooks: SccTarjanRecHooks = {
    onDiscover: (v, d) => {
      dfn.set(v, d);
      low.set(v, d);
      cur = v;
  ...
  sccTarjanRecursive(input, hooks);
