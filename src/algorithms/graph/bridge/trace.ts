// =============================================================================
// 桥 · 录制帧序列
// 可视化：setGraph（节点+边），role: 桥='final'，当前考察='compare'，已访问='frontier'；
// setAux 展示 dfn / low。
// =============================================================================

import type { BarRole, Frame, GraphEdge, GraphNode } from '../../../types.ts';
import { TraceRecorder } from '../../../core/recorder.ts';
import { bridge, edgeKey, type BridgeHooks, type GraphInput } from './impl.ts';

/** 演示用无向图：两个三角环由桥 (2-3) 相连 → 仅 1 条桥。
...
export function buildTrace(input: GraphInput = DEFAULT_INPUT): Frame[] {
  ...
  cur = null;
  rec
    .begin({ zh: `完成：桥 ${bridges.size} 条`, en: `Done: ${bridges.size} bridge(s)` })
    .setGraph(
      nodeIds.map((id) => ({
        id,
        label: id,
    ...
    .setAux([{ label: '桥 / bridges', value: [...bridges].join(', ') || '∅', role: 'final' }])
    .commit();
