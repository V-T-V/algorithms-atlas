// =============================================================================
// 割点 · 录制帧序列
// 可视化：setGraph（节点+边），role: 割点='pivot'，当前考察='compare'，已访问='frontier'；
// setAux 展示 dfn / low。
// =============================================================================

import type { BarRole, Frame, GraphEdge, GraphNode } from '../../../types.ts';
import { TraceRecorder } from '../../../core/recorder.ts';
import { cutVertex, type CutVertexHooks, type GraphInput } from './impl.ts';

/** 演示用无向图：两个三角环由桥 (2-3) 相连 → 割点为 2 与 3。
...
export function buildTrace(input: GraphInput = DEFAULT_INPUT): Frame[] {
  ...
  const render = (note: { zh: string; en: string }): void => {
    const nodes: GraphNode[] = nodeIds.map((id) => {
      ...
      if (visited.has(id) && !cut.has(id)) role = 'frontier';
  ...
  const hooks: CutVertexHooks = {
    ...
    onExamine: (u, v, kind) => {
      ...
      render({ zh: `考察 ${u}→${v}（${map[kind]}）`, en: `Examine ${u}→${v} (${mapEn[kind]})` });
      examEdge = null;
    },
    ...
    onCutVertex: (u, reason) => {
      ...
      render({ zh: `${u} 是割点（${r}）`, en: `${u} is a cut vertex (${re})` });
    },
  };

  cutVertex(input, hooks);

  // 终态
  cur = null;
  rec
    .begin({ zh: `完成：割点 ${cut.size} 个`, en: `Done: ${cut.size} cut vertex/vertices` })
    .setGraph(
      nodeIds.map((id) => ({
        id,
        label: id,
