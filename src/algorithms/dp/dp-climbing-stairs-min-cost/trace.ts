...
import { climbingStairsMinCost, type StairsMinCostHooks } from './impl.ts';

export const DEFAULT_INPUT = [1, 100, 1, 1, 1, 100, 1, 1, 100, 1];
...
export function buildTrace(input: readonly number[] = DEFAULT_INPUT): Frame[] {
  ...
  const snap = (note: { zh: string; en: string }): void => {
    ...
    rec
      ...
      .setBars(bars)
      .setAux([
        { label: 'dp(滚动)', value: dp.map((v) => (v < 0 ? '·' : v)).join(' '), role: 'compare' },
      ])
      .commit();
  };
  ...
  const hooks: StairsMinCostHooks = {
    onStep: (i, val) => {
      dp[i] = val;
      cur = i;
  ...
  climbingStairsMinCost(input, hooks);
