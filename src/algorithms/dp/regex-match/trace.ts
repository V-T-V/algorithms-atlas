...
import { regexMatch, type RegexMatchHooks } from './impl.ts';

export const DEFAULT_INPUT: { s: string; p: string } = { s: 'aab', p: 'c*a*b' };
...
export function buildTrace(input: { s: string; p: string } = DEFAULT_INPUT): Frame[] {
  ...
  snap({ zh: `匹配 s="${s}" 与 p="${p}"`, en: `Match s="${s}" against p="${p}"` });

  const hooks: RegexMatchHooks = {
    onFillCell: (i, j, val) => {
      dp[i]![j] = val;
      curI = i;
      curJ = j;
  ...
  const result = regexMatch(s, p, hooks);
